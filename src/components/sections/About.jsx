...
import { HiSearch, HiCalendar, HiLocationMarker, HiEye, HiThumbUp, HiX, HiShare, HiSortAscending } from 'react-icons/hi'
...
export default function About() {
  ...
  // Load news initially and subscribe to updates
  useEffect(() => {
    loadNews()
  ...
  const handleLike = (id) => {
    try {
      ...
      // Check if it is a local storage news or initialNews
      ...
      if (isLocal) {
        ...
        // If it belongs to initialNews, we can save its customized likes in localStorage
        // To make it easy, we store the full modified list of news in localStorage or just a local override list.
        // Let's just store the modified initialNews in localStorage if it gets liked
        const mergedNews = [...localNews, ...initialNews]
        const modified = mergedNews.map((n) => {
          if (n.id === id) return { ...n, likes: n.likes + 1 }
          return n
        })
        // Separate the locally created news from the seed news
        const localCreatedOnly = modified.filter((n) => !initialNews.some((init) => init.id === n.id))
        // And also store seed overrides if needed. Let's just store all in eco-news as an accumulated list!
        // That makes it extremely robust:
        const seedOverrides = modified.filter((n) => initialNews.some((init) => init.id === n.id))
        // Save the updated list containing local news plus overrides
      ...
      // Reload news and update modal state
      loadNews()
  ...
  const filteredNews = news
    ...
    .sort((a, b) => {
      ...
      return new Date(b.date) - new Date(a.date) // 'date' (default)
    })
  ...
  return (
    <section id="noticias" className="py-20 bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-md border border-gray-100 dark:border-gray-800/50 p-6 mb-10 space-y-6">
          <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
            ...
            <div className="flex items-center gap-2 w-full md:w-auto shrink-0 justify-end">
              ...
              <span className="text-xs text-gray-500 dark:text-gray-400 hidden sm:inline">{t('feed.sort_by')}:</span>
              <select
                value={sortBy}
          ...
          {/* Categories Tab */}
          <div className="flex flex-wrap gap-2 border-t border-gray-100 dark:border-gray-800/80 pt-4">
            {['all', 'ecologico', 'medioambiente', 'conservacion', 'liderazgo'].map((cat) => (
              <button
                key={cat}
          ...
          <AnimatePresence mode="popLayout">
            {filteredNews.map((item, idx) => (
              <motion.article
                ...
                className="bg-white dark:bg-gray-900 rounded-2xl overflow-hidden shadow-lg border border-gray-100 dark:border-gray-800/50 hover:shadow-xl dark:hover:border-gray-700/60 hover:-translate-y-1 transition-all duration-300 flex flex-col h-full"
              >
                {/* Image */}
                ...
                <div className="p-6 flex flex-col justify-between flex-grow">
                  ...
                  <div className="mt-6 pt-4 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
                    <div className="flex items-center gap-3">
                      ...
                      >
                        <HiThumbUp className="w-4 h-4 text-gray-400 hover:text-green-500" />
                        {item.likes}
                      </button>
                    </div>
